...
public class HorarioAtencionController {
    ...
    @PostMapping(consumes = {MediaType.APPLICATION_JSON_VALUE},
            ...
            value = "/post/registrarHorarioMedico")
    @ResponseBody
    public int registrarHorarioMedico(@RequestBody String pv_datos){
        ...
        try {
            ...
            LocalTime pt_hora_inicio = LocalTime.parse(job.get("pt_hora_inicio").toString(), DateTimeFormatter.ofPattern("HH:mm"));
            LocalTime pt_hora_fin = LocalTime.parse(job.get("pt_hora_fin").toString(), DateTimeFormatter.ofPattern("HH:mm"));
            LocalDate pd_fecha = LocalDate.parse(job.get("pd_fecha").toString());
            System.out.println(pv_datos);
            idHorario = horarioAtencionService.registrarHorarioMedico(pn_id_medico, pt_hora_inicio, pt_hora_fin, pd_fecha);
