...
const SearchPatientModal = ({ show, onClose, onSelect }) => {
  ...
  const fetchData = async (filtro) => {
    try {
      ...
      console.log(
        "No se pudo obtener los datos de los pacientes para el modal"
      );
    } finally {
      setLoading(false);
    }
            ...
            <div className="w-full max-h-screen overflow-y-auto rounded-md bg-white px-4 pt-3">
              {searchResults.slice(0, 6).map((result, index) => (
                ...
                >
                  ...
                  <p className="text-sm text-black">{result.dni}</p>
                </div>
              ))}
              {loading && (
          ...
          {selectedPatient && (
            <div className="py-2 px-3">
              ...
              <p className="text-sm text-black">
                Nombre: {selectedPatient.nombres}{" "}
                {selectedPatient.apellidoPaterno}{" "}
                {selectedPatient.apellidoMaterno}
              </p>
              <p className="text-sm text-black">DNI: {selectedPatient.dni}</p>
            </div>
          )}
        </div>
      </Modal.Footer>
