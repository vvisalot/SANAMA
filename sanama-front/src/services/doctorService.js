...
export const ENDPOINTS = {
  ...
  HORARIOS_POR_MEDICO_E_INTERVALO_FECHAS:
    "/rrhh/post/horarios_por_medico_e_intervaloFechas",
...
export const doctorService = {
  actualizarMedicoShort: async (data) => {
    try {
      const response = await axiosInstance.put(
        ENDPOINTS.ACTUALIZAR_MEDICO_SHORT,
        data
      );
  ...
  obtenerHorariosPorMedicoEIntervaloFechas: async (requestData) => {
    try {
      const response = await axiosInstance.post(
        ENDPOINTS.HORARIOS_POR_MEDICO_E_INTERVALO_FECHAS,
        requestData
      );
      return response.data;
    } catch (error) {
      console.error(
        "Error al obtener horarios por médico e intervalo de fechas",
        error
      );
      throw error;
    }
  ...
  registrarHorarioMedico: async (horarioData) => {
    try {
      const response = await axiosInstance.post(
        ENDPOINTS.REGISTRAR_HORARIO_MEDICO,
        horarioData
      );
      return response.data;
    } catch (error) {
  ...
  modificarDoctor: async (params) => {
    try {
      const response = await axiosInstance.put(
        "/rrhh/put/actualizarMedicoShort",
        params
      );
      return response.data;
    } catch (error) {
